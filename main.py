...
from lib import primestone_config
import time
import datetime
import sys
import os
...
def fix_masternode(data_folder):
    try:
        ...
        if not os.path.isfile(wallet_file):
            print('It seems like the data folder (the one containing wallet.dat) is not the same as the folder where primestone.conf is')
            data_folder = input('Please, write the data folder path: ')
            return fix_masternode(data_folder)

        print(colored('Make a copy of "wallet.dat" and "primestone.conf" before continuing.\nThis program will try it best not to touch them, but just in case!', attrs=['bold']))
        ...
        for filename in os.listdir(data_folder):
            if filename in ('wallet.dat', 'primestone.conf'):
                continue
        ...
        print(colored('Checking primestone.conf contents', 'green'))
        tokens = primestone_config.PrimeStoneConfig.tokenize(config.primestone_conf)
        ...
        tokens['rpcport'] = tokens.get('rpcport', 19984)
        tokens['server'] = 1
        ...
        try:
            ...
            r = None
            while r not in ('y', 'n'):
                print(colored('It seems like your primestone.conf is not setting the wallet as masternode!', 'red'))
        ...
        if 'masternodeprivkey' not in tokens:
            print(colored('Your primestone.conf does not contain a \'masternodeprivkey\' entry, please set it up before opening your wallet', 'red'))

        # Rewrite config
        with open(config.primestone_conf, 'w') as fp:
            for k, v in tokens.items():
                fp.write('{}={}\n'.format(k, v))

        print(colored('Done checking\n', 'green'))
        print(colored('Please open your wallet (primestoned on linux) and wait for it completely load/open before continuing', attrs=['bold']))
        ...
        print(colored('If it says it can not connect, check your primestone.conf settings', 'yellow'))
...
if __name__ == '__main__':
    ...
    print(colored('Using primestone.conf: {}'.format(config.primestone_conf), 'green'))
    ...
    if option == 1: 
        ...
        data_folder = os.path.dirname(config.primestone_conf)
