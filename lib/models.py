...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, primestoned):
        golist = primestoned.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_primestoned(primestoned, item)
    ...
    @classmethod
    def import_gobject_from_primestoned(self, primestoned, rec):
        import decimal
        ...
        # shim/primestoned conversion
        object_hex = primestonelib.SHIM_deserialise_from_primestoned(object_hex)
        objects = primestonelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from primestoned...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from primestoned, with every run
        ...
        # get/create, then sync payment amounts, etc. from primestoned - PrimeStoned is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(primestoned)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from primestoned! %s" % e)
            govobj.vote_delete(primestoned)
            return (govobj, None)
    ...
    def vote_delete(self, primestoned):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(primestoned, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, primestoned, signal, outcome):
        import primestonelib
        ...
        output = primestoned.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = primestonelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(primestoned, signal)

    def sync_network_vote(self, primestoned, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = primestoned.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = PRIMESTONED_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 primestone addr, non-multisig
            if not primestonelib.is_valid_primestone_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid PrimeStone address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (primestonelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/PrimeStoneDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import primestonelib
        obj_data = primestonelib.SHIM_serialise_for_primestoned(self.serialise())
    ...
    def prepare(self, primestoned):
        try:
            object_hash = primestoned.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = PRIMESTONED_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not primestonelib.is_valid_primestone_address(addr, config.network):
    ...
    def hash(self):
        import primestonelib
        return primestonelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = PRIMESTONED_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, primestoned):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - primestoned.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, primestoned):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - primestoned.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, primestoned):
        now = int(time.time())
        return (self.created_at < (now - primestoned.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, primestoned):
        if self.is_expired(primestoned):
            return False

        return True

    def is_deletable(self, primestoned):
        if self.is_expired(primestoned):
            return True
