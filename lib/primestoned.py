...
class PrimeStoneDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some primestoned methods
    ...
    @classmethod
    def from_primestone_conf(self, primestone_dot_conf):
        from primestone_config import PrimeStoneConfig
        config_text = PrimeStoneConfig.slurp_config_file(primestone_dot_conf)
        creds = PrimeStoneConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_current_masternode_vin(self):
        from primestonelib import parse_masternode_status_vin
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = primestonelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = primestonelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = primestonelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_primestoned_proto_version_with_sentinel_ping)
