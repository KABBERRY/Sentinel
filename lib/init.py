...
def has_primestone_conf():
    import config
    import io

    valid_primestone_conf = False

    # ensure primestone_conf exists & readable
    #
    # if not, print a message stating that PrimeStone Core must be installed and
    # configured, including JSONRPC access in primestone.conf
    try:
        f = io.open(config.primestone_conf)
        valid_primestone_conf = True
    except IOError as e:
        print(e)

    return valid_primestone_conf
...
def main():
    ...
    if not has_primestone_conf():
        print("PrimeStoneCore must be installed and configured, including JSONRPC access in primestone.conf")
