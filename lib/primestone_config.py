...
class PrimeStoneConfig():

    @classmethod
    def slurp_config_file(self, filename):
        # read primestone.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
            if re.match('^\s*#', line):
        ...
        # data is primestone.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from primestone.conf
        ...
        if args.rpc_port:
            ...
            # standard PrimeStone defaults...
            default_port = 9918 if (network == 'mainnet') else 19918

            # use default port for network if not specified in primestone.conf
            if not ('port' in creds):
                creds[u'port'] = default_port

            # convert to an int if taken from primestone.conf
